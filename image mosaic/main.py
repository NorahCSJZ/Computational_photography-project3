...
for H, imGray, imRGB, f in zip(Hs, imagesGray, imagesRBG, imageNames):
    ...
    # warp mask
    mask = imGray * 0 + 1
    maskWarped = (utils.warpImage(mask*255, Hshift @ H, targetImageSize) > 128)*1.0
...
mask = imFixGray * 0 + 1
imFixMaskWarped = (utils.warpImage(mask*255, Hshift, targetImageSize) > 128)*1.0
...
k = utils.calculateIntrinsics(imageMosaic.shape[1], imageMosaic.shape[0], 200)
imgCylindrical = utils.cylindricalWarp(np.dstack((imageMosaic, 255 * (Gains > 0))), k)
cv2.imwrite('./Results/cylindricalProjection.png', imgCylindrical)
